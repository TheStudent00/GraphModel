...
# ============================================================
# PART 1: PHYSICS PRIMITIVES (Trust & Realization)
# ============================================================
...
class SplineRealizer(nn.Module):
    """
    [The Observer]
    Converts Abstract Physics (Coeffs) -> Concrete Reality (Values).
    1. Superposition: Collapses Time (Long Exposure).
    2. Strips Physics: Drops Sigma and Z-Index.
    3. Realizes: Samples the curves.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Input: (B, T, Segments, 6)

        # 1. Superposition (Global Average Pooling across Time)
        # We average to keep magnitude stable regardless of sequence length.
        # Shape: (B, Segments, 6)
        pooled = x.mean(dim=1)

        # 2. Strip Physics (Keep only Coeffs)
        # We discard Sigma and Z-Index here. The Head doesn't need them.
        coeffs = pooled[..., :4] # (B, Segments, 4)

        # 3. Realize (Sample the curves)
        # (B, S, 4) @ (4, R) -> (B, S, R)
        realized = torch.matmul(coeffs, self.T_mat.t())

        # 4. Flatten for the Head
        # Output: (B, Segments * Resolution)
        return realized.view(realized.shape[0], -1)

class Atom(nn.Module):
    """
    [V11 Update]
    Passes Z-Index through the Warp untouched.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, T, M, 6) -> [Coeffs(4) | Sigma(1) | Z(1)]
        B, T, M, C = x.shape
        N = self.out_segments

        # 1. Resample
        if M != N:
            flat_x = x.view(B * T, M, C).transpose(1, 2)
...
class Compound(nn.Module):
    """
    [V11 Physics Engine]
    Implements Logarithmic Atmospheric Fade.
    """
    def __init__(self, dna_config):
        ...
        # Atmospheric Density (The "Haze" Parameter)
...
# ============================================================
# PART 3: THE EYE (Retina with Residuals & Z-Index)
# ============================================================

class VisualRetina(nn.Module):
    """
    [V11 Physics Update - FIXED]
    Outputs: [c3, c2, c1, c0, RMSE, Z_Index]
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 3. Z-Index Calculation (GPS) [FIXED]
        ...
        token_z_pos = z_mean_pixel.unsqueeze(-1) # (B, T, 1)
        ...
        return final_stream.unsqueeze(2) # (B, T, 1, 6)
...
class VisionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        # Realize (Superposition -> Strip Physics -> Sample)
        # Returns (B, N * Resolution)
        concept = self.realizer(thought)
...
class TrainingPlotter:
    ...
    def _plot(self):
        ...
        plt.title('Vision V11 (Atmospheric Fade)')
        fig.tight_layout()
...
if __name__ == "__main__":
    ...
    print("Initializing Vision V11 (Atmospheric Fade)...")
